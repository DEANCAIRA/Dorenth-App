...
\"\"\"Returns embedded financial data for 10 major Indonesian listed
companies\"\"\"
...
0: \[0, \"Year\", \"EBITDA\", \"Revenue\", \"Net Income\", \"Total
Assets\", \"Total Debt\", \"ROA %\", \"ROE %\", \"NIM %\"\],

1: \[0, \"\", \"EBITDA\", \"Revenue\", \"Net Income\", \"Total Assets\",
\"Total Debt\", \"ROA %\", \"ROE %\", \"NIM %\"\],
...
0: \[0, \"Year\", \"EBITDA\", \"Revenue\", \"Net Income\", \"Total
Assets\", \"Total Debt\", \"ROA %\", \"ROE %\", \"Operating Margin
%\"\],

1: \[0, \"\", \"EBITDA\", \"Revenue\", \"Net Income\", \"Total Assets\",
\"Total Debt\", \"ROA %\", \"ROE %\", \"Operating Margin %\"\],
...
Compare financial metrics across Indonesia\'s top 10 listed companies on
IDX (Indonesia Stock Exchange).

This tool includes real financial data from major Indonesian
corporations across banking, automotive, telecommunications,
...
st.success(\"✅ Using financial data from top 10 Indonesian listed
companies\")
...
\- \*\*PT Bank Rakyat Indonesia (BBRI)\*\* - Indonesia\'s largest bank
by assets
...
\- \*\*PT Astra International (ASII)\*\* - Automotive and heavy
...
\- \*\*PT Telkom Indonesia (TLKM)\*\* - National telecommunications
provider

\*\*Consumer Goods & Food:\*\*

\- \*\*PT Indofood Sukses Makmur (INDF)\*\* - Food and beverage
manufacturer
...
\*\*Available Metrics\*\*: EBITDA, Revenue, Net Income, Total Assets,
Total Debt, ROA%, ROE%, NIM%, Operating Margin%
...
search_input = st.text_input(\"🔍 Search for a metric (e.g., EBITDA,
Revenue, ROA)\").lower()

filtered_metrics = \[m for m in all_metrics if search_input in m\] if
search_input else all_metrics
...
clean_name = name.replace(\"PT\_\", \"\").replace(\"\_\", \" \")
...
tab1, tab2, tab3, tab4, tab5 = st.tabs(\[\"📈 Chart View\", \"📊 Table
View\", \"📋 Summary\", \"🏆 Rankings\", \"🔍 Quick Recall\"\])
...
yaxis_title=f\"{selected_metric.upper()} (in Trillions IDR)\" if
\"ebitda\" in selected_metric.lower() or \"revenue\" in
selected_metric.lower() else selected_metric.upper(),
...
file_name=f\"Indonesian_Companies\_{selected_metric}\_comparison.csv\",
...
growth = ((values.iloc\[-1\] - values.iloc\[0\]) / values.iloc\[0\] \*
100)
...
st.write(f\"{medals\[idx\]} \*\*{row\[\'Company\'\]}\*\*:
{row\[\'Value\'\]:.2f}\")

else:

st.write(f\"{row\[\'Rank\'\]}. \*\*{row\[\'Company\'\]}\*\*:
{row\[\'Value\'\]:.2f}\")
...
st.subheader(f\"🔍 Quick Recall: {selected_metric.upper()} for All
Companies\")

st.write(\"Instantly view the selected metric for all 10 Indonesian
companies in a specific year.\")
...
recall_data\[\"Value\"\] = pd.to_numeric(recall_data\[\"Value\"\],
errors=\"coerce\")
...
year_data = year_data.sort_values(\"Value\",
ascending=False).reset_index(drop=True)

st.markdown(f\"### 📊 {selected_metric.upper()} Rankings for Year
{recall_year}\")

st.write(f\"\*All 10 Indonesian companies ranked by
{selected_metric.upper()} in {recall_year}\*\")
...
elif selected_metric.lower() in \[\'ebitda\', \'revenue\', \'net
...
with st.expander(f\"📋 Historical Data: {selected_metric.upper()}
(2020-2024)\"):
...
file_name=f\"Indonesian_Companies\_{selected_metric}\_Historical_Data.csv\",
...
st.warning(f\"⚠️ No data available for year {recall_year}. Please select
a different year.\")

else:

st.error(\"❌ No data available for quick recall. Please check the data
source.\")

else:

st.warning(\"⚠️ No valid numerical data found for the selected metric
and filters.\")

else:

st.warning(\"⚠️ No matching data found for the selected metric across
companies.\")

else:

st.error(\"❌ No metrics found in the data. Please check your data
format.\")
...
\"Banking\": \[\"Bank Rakyat Indonesia BBRI\", \"Bank Central Asia
BBCA\", \"Bank Mandiri BMRI\"\],

\"Industrial\": \[\"Astra International ASII\", \"Semen Indonesia
SMGR\"\],

\"Consumer Goods\": \[\"Indofood Sukses Makmur INDF\", \"Unilever
Indonesia UNVR\", \"Gudang Garam GGRM\"\],

\"Infrastructure\": \[\"Telkom Indonesia TLKM\", \"Pertamina PGAS\"\]

}

sector_analysis = st.selectbox(\"Select sector for analysis:\",
list(sectors.keys()))
...
sector_data = plot_df\[plot_df\[\"Company\"\].isin(sector_companies)\]
if \'plot_df\' in locals() else pd.DataFrame()
...
st.info(f\"No data available for {sector_analysis} sector with current
filters.\")
...
st.markdown(\"🇮🇩 \*\*Indonesian Companies Financial Analysis\*\* \| Data
includes top IDX listed companies\")
