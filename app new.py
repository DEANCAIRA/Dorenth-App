...
def get_embedded_data():
    ...
    df_bri = pd.DataFrame({
        ...
        1: [0, 2020, 45.2, 98.5, 25.1, 1456.0, 1205.8, 1.72, 11.2, 6.8],
        2: [0, 2021, 52.8, 112.3, 31.4, 1635.7, 1351.2, 1.92, 12.5, 7.1],
        3: [0, 2022, 58.3, 125.7, 35.2, 1785.4, 1468.9, 1.97, 13.1, 7.3],
        4: [0, 2023, 62.1, 138.9, 38.8, 1925.3, 1589.7, 2.01, 13.8, 7.5],
        5: [0, 2024, 68.5, 152.4, 42.3, 2087.2, 1721.3, 2.03, 14.2, 7.7]
    })
    
    # PT Bank Central Asia Tbk (BBCA) - Private bank
    df_bca = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "NIM %"],
        1: [0, 2020, 38.7, 75.2, 28.1, 1021.8, 825.4, 2.75, 15.8, 6.2],
        2: [0, 2021, 42.3, 82.6, 31.3, 1152.3, 931.7, 2.72, 16.1, 6.4],
        3: [0, 2022, 45.9, 89.7, 34.2, 1268.5, 1024.8, 2.70, 16.5, 6.5],
        4: [0, 2023, 49.8, 97.3, 37.1, 1398.2, 1129.3, 2.65, 16.8, 6.7],
        5: [0, 2024, 53.2, 104.8, 39.8, 1521.6, 1228.1, 2.62, 17.2, 6.8]
    })
    
    # PT Bank Mandiri Tbk (BMRI) - State-owned bank
    df_mandiri = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "NIM %"],
        1: [0, 2020, 41.5, 89.3, 22.2, 1598.2, 1321.8, 1.39, 9.8, 5.9],
        2: [0, 2021, 46.2, 96.8, 26.1, 1742.5, 1438.7, 1.50, 10.5, 6.1],
        3: [0, 2022, 51.3, 105.7, 29.8, 1891.3, 1563.2, 1.58, 11.2, 6.3],
        4: [0, 2023, 55.7, 114.2, 32.9, 2034.7, 1681.5, 1.62, 11.8, 6.5],
        5: [0, 2024, 59.8, 122.3, 35.4, 2187.9, 1808.1, 1.62, 12.1, 6.6]
    })
    ...
    df_astra = pd.DataFrame({
        ...
        1: [0, 2020, 28.5, 185.9, 12.8, 295.4, 98.2, 4.3, 8.2, 12.5],
        2: [0, 2021, 32.1, 206.7, 16.2, 321.8, 105.7, 5.0, 9.8, 13.2],
        3: [0, 2022, 35.8, 228.4, 18.9, 348.9, 112.5, 5.4, 11.1, 13.8],
        4: [0, 2023, 38.2, 245.6, 20.7, 372.1, 118.9, 5.6, 11.8, 14.1],
        5: [0, 2024, 41.3, 263.2, 22.5, 396.7, 125.3, 5.7, 12.2, 14.5]
    })
    
    # PT Telkom Indonesia Tbk (TLKM) - Telecommunications
    df_telkom = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 22.8, 64.1, 15.3, 189.2, 42.7, 8.1, 12.5, 32.1],
        2: [0, 2021, 24.5, 67.8, 16.8, 201.5, 45.3, 8.3, 13.2, 33.2],
        3: [0, 2022, 26.3, 71.9, 18.2, 214.8, 48.1, 8.5, 13.8, 34.1],
        4: [0, 2023, 27.9, 75.4, 19.5, 227.3, 50.2, 8.6, 14.2, 34.8],
        5: [0, 2024, 29.6, 79.1, 20.8, 240.1, 52.4, 8.7, 14.6, 35.2]
    })
    
    # PT Indofood Sukses Makmur Tbk (INDF) - Food & Beverage
    df_indofood = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 18.7, 76.6, 4.2, 89.3, 35.8, 4.7, 8.9, 18.5],
        2: [0, 2021, 20.3, 82.1, 5.1, 94.7, 37.2, 5.4, 10.2, 19.2],
        3: [0, 2022, 21.8, 87.9, 5.8, 100.2, 38.9, 5.8, 11.1, 19.8],
        4: [0, 2023, 23.1, 92.8, 6.3, 105.8, 40.1, 6.0, 11.7, 20.2],
        5: [0, 2024, 24.6, 98.4, 6.9, 111.9, 41.5, 6.2, 12.3, 20.7]
    })
    
    # PT Unilever Indonesia Tbk (UNVR) - Consumer Goods
    df_unilever = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 8.9, 41.2, 7.2, 18.5, 2.1, 38.9, 85.2, 19.8],
        2: [0, 2021, 9.5, 43.8, 7.8, 19.8, 2.3, 39.4, 88.1, 20.5],
        3: [0, 2022, 10.1, 46.7, 8.3, 21.2, 2.5, 39.2, 89.7, 21.1],
        4: [0, 2023, 10.6, 48.9, 8.7, 22.4, 2.6, 38.8, 91.2, 21.4],
        5: [0, 2024, 11.2, 51.5, 9.2, 23.8, 2.8, 38.7, 92.8, 21.8]
    })
    
    # PT Gudang Garam Tbk (GGRM) - Tobacco
    df_gudang_garam = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 12.8, 68.7, 6.9, 67.8, 15.2, 10.2, 18.5, 16.2],
        2: [0, 2021, 14.2, 73.9, 8.1, 72.1, 16.8, 11.2, 20.8, 17.1],
        3: [0, 2022, 15.6, 79.4, 9.2, 76.8, 18.3, 12.0, 22.7, 17.8],
        4: [0, 2023, 16.8, 84.2, 10.1, 81.2, 19.7, 12.4, 24.1, 18.3],
        5: [0, 2024, 18.1, 89.6, 11.0, 85.9, 21.2, 12.8, 25.2, 18.9]
    })
    
    # PT Semen Indonesia Tbk (SMGR) - Cement
    df_semen = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 8.2, 28.9, 1.8, 48.7, 18.9, 3.7, 6.2, 22.5],
        2: [0, 2021, 9.1, 31.5, 2.3, 51.2, 19.8, 4.5, 7.8, 23.8],
        3: [0, 2022, 9.8, 34.2, 2.9, 54.1, 20.7, 5.4, 9.5, 24.7],
        4: [0, 2023, 10.3, 36.1, 3.2, 56.8, 21.4, 5.6, 10.1, 25.2],
        5: [0, 2024, 10.9, 38.4, 3.6, 59.7, 22.3, 6.0, 11.2, 25.8]
    })
    
    # PT Pertamina Tbk (PGAS) - Oil & Gas
    df_pertamina = pd.DataFrame({
        0: [0, "Year", "EBITDA", "Revenue", "Net Income", "Total Assets", "Total Debt", "ROA %", "ROE %", "Operating Margin %"],
        1: [0, 2020, 15.6, 42.3, 2.1, 78.9, 28.7, 2.7, 4.8, 28.9],
        2: [0, 2021, 18.9, 51.7, 3.8, 85.2, 31.2, 4.5, 8.2, 32.1],
        3: [0, 2022, 22.3, 63.8, 5.7, 92.4, 33.9, 6.2, 11.5, 34.8],
        4: [0, 2023, 24.7, 71.2, 7.1, 98.7, 35.8, 7.2, 13.8, 36.2],
        5: [0, 2024, 26.8, 77.9, 8.2, 104.8, 37.5, 7.8, 15.1, 37.4]
    })
...
def get_available_metrics(dataframes):
    ...
    for df in dataframes.values():
        if len(df.columns) > 1:
            ...
            labels = df.iloc[0, 1:].astype(str).str.lower().str.strip().tolist()
...
def extract_metric(df, metric_name):
    ...
    try:
        # Get years from column 1, rows 1-5 (where the actual data is)
        years = []
        values = []
